...
# a. Comparar diferentes modelos.
# No es necesario que el preprocesamiento de los datos se haga con PyCaret, se pueden usar las otras librerías.

clf = setup(data=heart_df, target='output', session_id=42, normalize=True)
best_model_pycaret = compare_models()
print("Mejor modelo según PyCaret:", best_model_pycaret)

# b. Definir los 3 mejores modelos hasta el momento y hacer la búsqueda de hiper parámetros de estos.
...
# Seleccionar el modelo con mejor métrica (ajustar criterio según prefieras)
final_model = max(models.items(), key=lambda x: roc_auc_score(y_test, x[1].predict_proba(X_test)[:, 1]))[1]
print("Modelo seleccionado como el mejor:", final_model)


# d. ¿Difieren los resultados respecto a lo hecho con Scikit Learn en el taller pasado?

# Generar predicciones del mejor modelo de PyCaret
predictions_pycaret = predict_model(best_model_pycaret, data=X_test)

# Access the predicted labels using the 'prediction_label' column instead of 'Label'